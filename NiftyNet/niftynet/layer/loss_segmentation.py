...
class LossFunction(Layer):
    ...
    def layer_op(self, prediction, ground_truth, is_brats=None, weight_map=None):
        """
...
def dice_transfert(prediction, ground_truth, weight_map=None):
    prediction = tf.cast(prediction, tf.float32)
    prediction_soft = tf.nn.softmax(prediction)

    ground_truth = tf.cast(ground_truth, tf.float32)
    mask =  tf.equal(ground_truth,0)
    mask = tf.cast(mask,tf.float32)
    n_classes = prediction.shape[1].value
    mask = tf.reshape(tf.tile(mask, [n_classes]), prediction.get_shape())

    prediction = tf.multiply(mask, prediction)
    prediction_soft = tf.multiply(mask, prediction_soft)
    #tf.Print(prediction.get_shape(),prediction.get_shape())

    dice_numerator = 2.0 * tf.reduce_sum(
            prediction * prediction_soft, axis=0)
    dice_denominator = \
            tf.reduce_sum(tf.square(prediction), axis=0) + \
            tf.reduce_sum(tf.square(prediction_soft), axis=0)
    epsilon_denominator = 0.00001
    dice_score = dice_numerator / (dice_denominator + epsilon_denominator)
    return 1.0 - tf.reduce_mean(dice_score[:4])

def dice_constraint(prediction, ground_truth, is_brats, weight_map=None):
    def swtich_loss(prediction, ground_truth, for_brats):
        prediction = tf.cast(prediction, tf.float32)
        prediction = tf.nn.softmax(prediction)

        ground_truth = tf.cast(ground_truth, tf.float32)
        ground_truth = tf.nn.softmax(ground_truth)
        #tf.Print(prediction.get_shape(),prediction.get_shape())

        dice_numerator = 2.0 * tf.reduce_sum(
                prediction * ground_truth, axis=0)
        dice_denominator = \
                tf.reduce_sum(tf.square(prediction), axis=0) + \
                tf.reduce_sum(tf.square(ground_truth), axis=0)
        epsilon_denominator = 0.00001
        dice_score = dice_numerator / (dice_denominator + epsilon_denominator)
        if for_brats:
            ...
            return 1.0 - tf.reduce_mean(dice_score[:4])
    return tf.cond(is_brats, lambda: swtich_loss(prediction, ground_truth, for_brats=True), lambda: swtich_loss(prediction, ground_truth, for_brats=False))
...
def jaccard(prediction, ground_truth, weight_map=None):
    prediction = tf.cast(prediction, tf.float32)
    if len(ground_truth.shape) == len(prediction.shape):
        ground_truth = ground_truth[..., -1]
    one_hot = labels_to_one_hot(ground_truth, tf.shape(prediction)[-1])
    ...
    jaccard_mul = tf.sparse_reduce_sum(
        one_hot * prediction, reduction_axes=[0])
    jaccard_sum = \
        tf.reduce_sum(tf.square(prediction), reduction_indices=[0]) + \
        tf.sparse_reduce_sum(one_hot, reduction_axes=[0]) 
    ...
    jaccard_score = (jaccard_mul + epsilon ) / (jaccard_sum - jaccard_mul + epsilon)
    return 1.0 - tf.reduce_mean(jaccard_score)



def jaccard_prob(prediction, ground_truth, weight_map=None):
    #tf.Print(prediction.get_shape(),prediction.get_shape())

    jaccard_mul = tf.reduce_sum(
            prediction * ground_truth, axis=0)
    jaccard_sum = \
            tf.reduce_sum(tf.square(prediction), axis=0) + \
            tf.reduce_sum(tf.square(ground_truth), axis=0)
...
def jaccard_lesion(prediction, ground_truth, weight_map=None):
    prediction = tf.cast(prediction, tf.float32)
    if len(ground_truth.shape) == len(prediction.shape):
        ground_truth = ground_truth[..., -1]
    one_hot = labels_to_one_hot(ground_truth, tf.shape(prediction)[-1])
    ...
    jaccard_score = (jaccard_mul + epsilon ) / (jaccard_sum - jaccard_mul + epsilon)
    return 1.0 - jaccard_score[-1]


def dice_binary_alone(prediction, ground_truth, weight_map=None):
    """
    Function to calculate the dice loss with the definition given in
    ...
    # dice_score.set_shape([n_classes])
    # minimising (1 - dice_coefficients)
    return 1.0 - tf.reduce_mean(dice_score[-1:])


def dice_brats(prediction, ground_truth, is_brats, weight_map=None):
    """
    Function to calculate the dice loss with the definition given in
    ...
    def swtich_loss(prediction, ground_truth, for_brats):
        if not for_brats:
            ...
            # minimising (1 - dice_coefficients)
            #dice_score[:4] = 0
            return 1.0 - tf.reduce_mean(dice_score[-3:])
            #return dice_score
    return tf.cond(is_brats, lambda: swtich_loss(prediction, ground_truth, for_brats=True), lambda: swtich_loss(prediction, ground_truth, for_brats=False))
        #return 1.0 - tf.reduce_mean(dice_score)


def dice_brats_mrbrains(prediction, ground_truth, is_brats, weight_map=None):
    """
    Function to calculate the dice loss with the definition given in
    ...
    def swtich_loss(prediction, ground_truth, for_brats):
        if not for_brats:
            ...
            dice_score = dice_numerator / (dice_denominator + epsilon_denominator)
            dice_score.set_shape([10])
            # dice_score.set_shape([n_classes])
            # minimising (1 - dice_coefficients)
            return 1.0 - tf.reduce_mean(dice_score[:7])
        else:
            prediction = tf.cast(prediction, tf.float32)
            if len(ground_truth.shape) == len(prediction.shape):
                ground_truth = ground_truth[..., -1]
...
def dice_brats2(prediction, ground_truth, is_brats, weight_map=None):
    """
    Function to calculate the dice loss with the definition given in
    ...
    def swtich_loss(prediction, ground_truth, for_brats, weight_map=None):
        if not for_brats:
            ...
            return 1.0 - tf.reduce_mean(dice_score)
            #return dice_score
    return tf.cond(is_brats, lambda: swtich_loss(prediction, ground_truth, for_brats=True, weight_map=weight_map), lambda: swtich_loss(prediction, ground_truth, for_brats=False, weight_map=weight_map))


def dice_brats_V2(prediction, ground_truth, is_brats, weight_map=None):
    """
    Function to calculate the dice loss with the definition given in
    ...
    def swtich_loss(prediction, ground_truth, for_brats):
        if not for_brats:
            ...
            dice_score = (dice_numerator + epsilon_denominator/2) / (dice_denominator + epsilon_denominator)
            dice_score.set_shape([7])
            # dice_score.set_shape([n_classes])
            # minimising (1 - dice_coefficients)
            return 1.0 - tf.reduce_mean(dice_score)
...
def dice_brats_uni(prediction, ground_truth, is_brats, weight_map=None):
    """
    Function to calculate the dice loss with the definition given in
    ...
    def swtich_loss(prediction, ground_truth, for_brats):
        if not for_brats:
            ...
            if len(ground_truth.shape) == len(prediction.shape):
                ground_truth = ground_truth[..., -1]
            one_hot = labels_to_one_hot(ground_truth, tf.shape(prediction)[-1]+1)
            one_hot = tf.sparse_to_dense(one_hot.indices, one_hot.dense_shape, one_hot.values, default_value=0)
            one_hot = one_hot[:,1:]
            mask = tf.cast(tf.equal(one_hot[:,:1],0),tf.float32)
            ...
            print(prediction)
            dice_numerator = 2.0 * tf.reduce_sum(
                one_hot * prediction, reduction_indices=[0])
            dice_denominator = \
                tf.reduce_sum(tf.square(prediction), reduction_indices=[0]) + \
...
def dice_brats_uni_prob(prediction, ground_truth, is_brats, weight_map=None):
    """
    Function to calculate the dice loss with the definition given in
    ...
    def swtich_loss(prediction, ground_truth, for_brats):
        if not for_brats:
            ...
            dice_numerator = 2.0 * tf.reduce_sum(
                ground_truth * prediction, reduction_indices=[0])
            dice_denominator = \
                tf.reduce_sum(tf.square(prediction), reduction_indices=[0]) + \
                tf.reduce_sum(ground_truth, reduction_indices=[0])
            epsilon_denominator = 0.00001
...
def jaccard_brats(prediction, ground_truth, is_brats, weight_map=None):
    """
    Function to calculate the dice loss with the definition given in
    ...
    def swtich_loss(prediction, ground_truth, for_brats):
        if not for_brats:
            ...
            dice_numerator = tf.sparse_reduce_sum(
                one_hot * prediction, reduction_axes=[0])
            dice_denominator = \
                tf.reduce_sum(tf.square(prediction), reduction_indices=[0]) + \
                tf.sparse_reduce_sum(one_hot, reduction_axes=[0])
            ...
            dice_score = dice_numerator / (dice_denominator - dice_numerator + epsilon_denominator)
            dice_score.set_shape([7])
            # dice_score.set_shape([n_classes])
            # minimising (1 - dice_coefficients)
            return 1.0 - tf.reduce_mean(dice_score[:4])
...
def jaccard_constraint(prediction, ground_truth, is_brats, weight_map=None):
    def swtich_loss(prediction, ground_truth, for_brats):
        prediction = tf.cast(prediction, tf.float32)
        prediction = tf.nn.softmax(prediction)
