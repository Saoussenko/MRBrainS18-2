...
class CRFAsRNNLayer(TrainableLayer):
    ...
    def layer_op(self, I, U, MODALITIES):
        """
        ...
        n_feat = I[list(I.keys())[0]].shape.as_list()[-1]
        ...
        if self._w_init is None:
            self._w_init = [np.ones(n_ch), np.ones(n_ch), np.ones(n_ch), np.ones(n_ch), np.ones(n_ch)]
        self._w_init = [
