...
class HighRes3DNet(BaseNet):
    ...
    def __init__(self,
        ...
        self.layers = [
            {'name': 'conv_0', 'n_features': 8, 'kernel_size': (3, 3, 3)},
            {'name': 'res_1', 'n_features': 16, 'kernels': (3, 3, 3), 'repeat': 1},
            {'name': 'res_2', 'n_features': 16, 'kernels': (3, 3, 3), 'repeat': 3},
            {'name': 'res_3', 'n_features': 16, 'kernels': (3, 3, 3), 'repeat': 3},
            {'name': 'conv_1', 'n_features': 32, 'kernel_size': (1, 1, 1)},
            {'name': 'conv_2', 'n_features': 8, 'kernel_size': (1, 1, 1)}]

    def layer_op(self, images, is_training, layer_id=-1):
        assert layer_util.check_spatial_dims(
            images['T1'], lambda x: x % 8 == 0)
        assert set(images.keys()).issubset(set(MODALITIES)), 'image has to be a dictionnary with keys in %s' % (MODALITIES)
        ...
        for mod in MODALITIES:
            ...
            for j in range(params['repeat']):
                res_block = HighResBlock(
                    ...
                    w_initializer=self.initializers['w'],
                    w_regularizer=self.regularizers['w'],
                    name='%s_%s_%d' % (params['name'],mod, j))
